...
const ActiveOption = ({ row, column, table }) => {
  ...
  return (
    <div
      className={`cursor-pointer relative w-20 grid justify-center px-2 py-1 font-sans text-xs font-bold uppercase rounded-md select-none whitespace-nowrap ${activeStyles[value]} ${
        disable ? "pointer-events-none opacity-90" : ""
      }`}
    >
      <select
        className={`min-w-16 text-sm border border-transparent py-1 transition duration-300 ease outline-none ${activeStyles[value]} ${
